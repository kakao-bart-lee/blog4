...
등이 될 것입니다. 결국 "{위치} + 진료소" 류가 될 텐데요, 위치가 같이 들어오지 않는다면 위치 정보를 받을 수 있는 기능을 연결해서 지역을 받아 스킬 서버로 요청 들어오도록 구성 했습니다.

어떻게든 지역 명을 받으면 [주소 검색 API](https://developers.kakao.com/docs/restapi/local#%EC%A3%BC%EC%86%8C-%EA%B2%80%EC%83%89)를 통해서 정제된 주소 데이터로 바꾼 후 다시 [키워드로 장소 검색 API](https://developers.kakao.com/docs/restapi/local#%ED%82%A4%EC%9B%8C%EB%93%9C-%EA%B2%80%EC%83%89)를 이용해 선별 진료소의 정보를 구했습니다.
...
인데요. 내부적으로는 지식+를 통해 나온 결과에도 score 값이 있고 의도 파악 로직에도 머신 러닝을 태운 결과라면 score 값이 있습니다. 문제는 외부에 오픈된 기능에서는 이것의 임계점을 설정할 수가 없어요. 

예를 들어 기계님이 "들어온 발화는 아마도 이 질문 같아요"라고 말씀해주시며 score 값이 100점 만점에 70점 정도인데 내부 임계점이 65점이면 요걸로 답이 나가버리는 거죠. 근데 이게 기획 의도와는 차이가좀 있어서 저는 지식+ 세팅을 끄고 내부적으로 호출한 뒤 score 값을 따로 설정해서 답변을 선택하게 했습니다. 
...
빠르게 사라져야할 봇이고 실제 봇 호출량도 감소 추세여서 다행이다 생각하고 있었는데 요즘 들어 다시 증가세네요. 

![블록 호출 추세](/media/call_trend.jpg)

질본 채널도 어느새 초기 30만이던 친구수가 어느덧 70만을 돌파 했습니다. 
